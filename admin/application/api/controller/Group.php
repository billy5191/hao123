...
public function index($page = 1) {
    if ($this->request->isPost()) {
        ...
        foreach ($keywords as $keyword) {
            $query->where('group_nick', 'like', '%' . $keyword . '%');
        }

        // 获取结果并排序
        $res = $query->order("group_id desc")->paginate(20);

        // 如果结果较少，尝试使用更加智能的模糊匹配
        if ($res->total() < 3) {
            // 将关键词拆分成若干部分
            $splitKeywords = $this->splitKeywords($keywords);

            // 重新构建查询
            $query = $group;

            // 逐个添加拆分后的关键词的模糊匹配条件
            foreach ($splitKeywords as $splitKeyword) {
                $query->whereLike('group_nick', '%' . $splitKeyword . '%', 'OR');
            }
...
private function splitKeywords($keywords) {
    ...
    foreach ($keywords as $keyword) {
        ...
        for ($i = 1; $i <= min(2, $length); $i++) {
            $splitKeywords[] = mb_substr($keyword, 0, $i);
        }
    }

    return array_unique($splitKeywords);
}
    ...
    public function writegroup($iscj = 1){
        if($this->request->isPost()) {
            ...
            //$res = $member->where(["group_username"=>$this->request->post("username")])->find();
            //if($res){
            //    $this->error(__("memberdatanull"));
            //}
            $arr = [
