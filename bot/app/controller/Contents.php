...
class Contents extends Base
{
    ...
    $groupNick = $data["data"][$i]["group_nick"];

    // 如果字符数超过20，截取前20个字符并添加省略号
    if (mb_strlen($groupNick, 'utf-8') > 20) {
        $groupNick = mb_substr($groupNick, 0, 20, 'utf-8') . '...';
    }
                ...
                $topadv .= $k . "." . $avtic . "<a href='https://t.me/{$data["data"][$i]["group_username"]}'>{$groupNick} </a> - {$s}\n";
            }
        }
        //获取系统配置中底部内容
